...
function catInfo(p){
  const cat = String((p && p.cat) || "").toLowerCase();
  const res = (p && p.section === "residential") || /^res-/.test(cat);
  if(res){
    if(/sofa/.test(cat)) return { label:"Home Sofas", shop:"residential" };
    if(/set/.test(cat)) return { label:"Dining Sets", shop:"residential" };
    if(/table/.test(cat)) return { label:"Dining Tables", shop:"residential" };
    if(/chair/.test(cat)) return { label:"Dining Chairs", shop:"residential" };
    return { label:"Residential Furniture", shop:"residential" };
  }
  if(/set/.test(cat)) return { label:"Restaurant Sets", shop:"restaurant-set" };
  if(/stool/.test(cat)) return { label:"Bar Stools", shop:"bar-stool" };
  if(/outdoor/.test(cat)) return { label:"Outdoor Chairs", shop:"outdoor-chair" };
  if(/booth|sofa/.test(cat)) return { label:"Booth & Sofas", shop:"booth" };
  if(/base/.test(cat)) return { label:"Table Bases", shop:"table-base" };
  if(/table/.test(cat)) return { label:"Restaurant Tables", shop:"table" };
  if(/chair/.test(cat)) return { label:"Indoor & Café Chairs", shop:"indoor-chair" };
